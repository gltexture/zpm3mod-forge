...
public interface IZPGunParticlesFX {
    ...
    ParticlesEmitter DEFAULT_PARTICLES_EMITTER = ((player, baseGun, itemStack, gunFXData) -> {
    ...
    ParticlesEmitter DEFAULT_PARTICLES_EMITTER_NO_SHELL = ((player, baseGun, itemStack, gunFXData) -> {
        for (int i = 0; i < Math.max(gunFXData.recoilStrength() / 3.0f, 1.0f) + ZPRandom.getRandom().nextInt(2); i++) {
            ZPDefaultGunParticlesFX.emmitParticleSmoke(gunFXData.isRightHand(), player, false);
        }
    });

    ParticlesEmitter DEFAULT_PARTICLES_EMITTER_NO_SMOKE = ((player, baseGun, itemStack, gunFXData) -> {
        ZPDefaultGunParticlesFX.emmitParticleShell(gunFXData.isRightHand(), player);
    });

    ParticlesEmitter DEFAULT_PARTICLES_EMITTER_SUPER_SMOKY_NO_SHELL = ((player, baseGun, itemStack, gunFXData) -> {
    ...
    interface ParticlesEmitter {
        void emmit(@NotNull Player player, @NotNull ZPBaseGun baseGun, @NotNull ItemStack itemStack, ZPClientCallbacks.ZPGunShotCallback.@NotNull GunFXData gunFXData);
    }
}
