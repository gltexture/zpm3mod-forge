...
public class ZPBetterVisionPostFXProcessor extends ZPPostFXProcessor{
    public ZPBetterVisionPostFXProcessor(int chainOrder) {
        super(chainOrder);
    }
    ...
    protected ZPShaderLoader.ZPShaderInstance getPostFXShader() {
        return ZPDefaultShaders.post_fx_bettervis;
    }

    @Override
    public boolean bypass() {
        if (Minecraft.getInstance().player != null) {
            return !Minecraft.getInstance().player.hasEffect(ZPMobEffects.better_vision.get());
        }
        return !ZPImGuiDebugInterface.FORCE_ENABLE_BETTERVIS_POST_FX_SHADER;
    }
}
