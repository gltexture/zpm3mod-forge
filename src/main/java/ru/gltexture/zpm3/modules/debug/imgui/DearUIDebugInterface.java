    ...
    public void drawGui(@NotNull Window window, @NotNull MouseHandler mouseHandler, @NotNull KeyboardHandler keyboardHandler) {
       ...
       // }
        ...
        if (ImGui.collapsingHeader("PostFX")) {
            ...
            if (ImGui.checkbox("Infection", DearUIDebugInterface.FORCE_ENABLE_INFECTION_POST_FX_SHADER)) {
                DearUIDebugInterface.FORCE_ENABLE_INFECTION_POST_FX_SHADER = !DearUIDebugInterface.FORCE_ENABLE_INFECTION_POST_FX_SHADER;
            }
            ImGui.dragFloat("RadParam", DearUIDebugInterface.PARAM_RAD_POSTFX, 0.01f, 0.0f, 100.0f);
            ImGui.dragFloat("InfParam", DearUIDebugInterface.PARAM_INF_POSTFX, 0.01f, 0.0f, 100.0f);
        ...
        if (ImGui.collapsingHeader("Gun-Rendering")) {
            ...
            if (ImGui.checkbox("manual mflash", DearUIDebugInterface.muzzleflashHandling)) {
                DearUIDebugInterface.muzzleflashHandling = !DearUIDebugInterface.muzzleflashHandling;
            }
            if (ImGui.checkbox("emmit smoke", DearUIDebugInterface.emmitSmoke)) {
                DearUIDebugInterface.emmitSmoke = !DearUIDebugInterface.emmitSmoke;
            }
            if (ImGui.checkbox("emmit shells", DearUIDebugInterface.emmitShells)) {
                DearUIDebugInterface.emmitShells = !DearUIDebugInterface.emmitShells;
            }
            int[] mode = new int[] {DearUIDebugInterface.muzzleflashRenderingMode};
            ImGui.sliderInt("Quality", mode, 0, 3);
            DearUIDebugInterface.muzzleflashRenderingMode = mode[0];
            ...
            if (ImGui.treeNode("Adjust Gun Render(1P)")) {
                ...
                float[] scissor = new float[] {DearUIDebugInterface.scissor1P};
                ImGui.sliderFloat("scissor", scissor, 0.0f, 1.0f);
                DearUIDebugInterface.scissor1P = scissor[0];

                float[] reload = new float[] {DearUIDebugInterface.reloadProgression};
                ImGui.sliderFloat("reload", reload, 0.0f, 1.0f);
                DearUIDebugInterface.reloadProgression = reload[0];

            //    float[] blur = new float[] {DearUIDebugInterface.muzzleFlash1PersonBlurring};
            //    ImGui.sliderFloat("blurring", blur, 1.0f, 12.0f);
            //    DearUIDebugInterface.muzzleFlash1PersonBlurring = blur[0];

                int[] operations = new int[] {DearUIDebugInterface.muzzleflash1PersonFboPingPongOperations};
                ImGui.sliderInt("FBO-Operations", operations, 0, 128);
                DearUIDebugInterface.muzzleflash1PersonFboPingPongOperations = operations[0];
                ImGui.treePop();
            }
            if (ImGui.treeNode("Adjust Gun Render(3P)")) {
                ...
                float[] scissor = new float[] {DearUIDebugInterface.scissor3P};
                ImGui.sliderFloat("scissor", scissor, 0.0f, 1.0f);
                DearUIDebugInterface.scissor3P = scissor[0];

              //  float[] blur = new float[] {DearUIDebugInterface.muzzleFlash3PersonBlurring};
              //  ImGui.sliderFloat("blurring", blur, 1.0f, 12.0f);
              //  DearUIDebugInterface.muzzleFlash3PersonBlurring = blur[0];

               // int[] operations = new int[] {DearUIDebugInterface.muzzleflash3PersonFboPingPongOperations};
               // ImGui.sliderInt("FBO-Operations", operations, 0, 128);
               // DearUIDebugInterface.muzzleflash3PersonFboPingPongOperations = operations[0];
                ImGui.treePop();
            }
        ...
        if (ImGui.collapsingHeader("Debug Darkness")) {
            if (ImGui.checkbox("Enable", DearUIDebugInterface.debugDarknessValueEnable)) {
                DearUIDebugInterface.debugDarknessValueEnable = !DearUIDebugInterface.debugDarknessValueEnable;
            }
            float[] v = new float[] {DearUIDebugInterface.debugDarknessValue};
        ...
        if (ImGui.collapsingHeader("Debug Params")) {
            ImGui.dragFloat("PARAM1", DearUIDebugInterface.PARAM1);
            ImGui.dragFloat("PARAM2", DearUIDebugInterface.PARAM2);
            ImGui.dragFloat("PARAM3", DearUIDebugInterface.PARAM3);
            ImGui.dragFloat("PARAM4", DearUIDebugInterface.PARAM4);
            ImGui.dragFloat("PARAM5", DearUIDebugInterface.PARAM5);
        }
        ImGui.end();
    }
