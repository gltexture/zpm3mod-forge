...
public class ZPBrickEntity extends ZPThrowableEntity {
    public ZPBrickEntity(EntityType<ZPBrickEntity> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public ZPBrickEntity(EntityType<ZPBrickEntity> pEntityType, double pX, double pY, double pZ, Level pLevel) {
        super(pEntityType, pX, pY, pZ, pLevel);
    }

    public ZPBrickEntity(EntityType<ZPBrickEntity> pEntityType, LivingEntity pShooter, Level pLevel) {
    ...
    public void handleEntityEvent(byte pId) {
        if (pId == 3) {
            this.level().playLocalSound(this.getOnPos(), SoundEvents.STONE_BREAK, SoundSource.MASTER, 0.8f, 1.0f, false);
    ...
    protected void onHitEntity(@NotNull EntityHitResult pResult) {
        ...
        if (!entity.level().isClientSide()) {
            entity.hurt(this.damageSources().thrown(this, this.getOwner()), ZPCombatConfig.BRICK_DAMAGE.getVar());
        }
    }
                ...
                if (!this.level().isEmptyBlock(pos)) {
                    if (this.level() instanceof IZPLevelExt ext) {
                        if (ZPFakePlayer.canBreakBlock((ServerLevel) this.level(), pos) && !ZPZoneChecks.INSTANCE.isNoThrowableBlockDamage((ServerLevel) this.level(), pos)) {
                            ext.zpm3forge$getGlobalBlocksDestroyMemory().addNewEntryLongMem(this, this.level(), pos, (1.0f + ZPRandom.getRandom().nextFloat(4.0f)) * ZPCombatConfig.THROWABLES_BLOCK_BREAK_MULTIPLIER.getVar());
