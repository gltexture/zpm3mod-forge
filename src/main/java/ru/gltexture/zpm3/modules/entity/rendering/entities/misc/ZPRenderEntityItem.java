...
package ru.gltexture.zpm3.modules.entity.rendering.entities.misc;
...
public class ZPRenderEntityItem extends ItemEntityRenderer {
    ...
    public void render(@NotNull ItemEntity pEntity, float pEntityYaw, float pPartialTicks, @NotNull PoseStack pPoseStack, @NotNull MultiBufferSource pBuffer, int pPackedLight) {
        ...
        final boolean pickUpOnKey = ZombiePlague3.netClient().getNetStaticDataSyncer().getVar(ZPNetPackModule.StoC__SERVER_PICK_UP_ON_KEY).orElse(new ZPNetDataBoolean(ZPWorldConfig.ALLOW_ITEMS_PICKING_ON_KEY.getVar())).getValue();
