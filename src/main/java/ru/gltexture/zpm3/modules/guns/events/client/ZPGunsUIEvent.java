...
@OnlyIn(Dist.CLIENT)
public class ZPGunsUIEvent implements ZPForgeEventHandlerClass {
    ...
    public static void exec(RenderGuiOverlayEvent.@NotNull Post event) {
        ...
        if (event.getOverlay() == VanillaGuiOverlay.CROSSHAIR.type()) {
            ...
            if (ZPClientGunClientTickProcessing.showLeftMouseIndicator(mc)) {
                ZPGunsUIEvent.tip(graphics, ZPGunsUIEvent.mouse_left, new Vector2i(center.x, 24).sub(21, 16), new Vector2i(16, 16), new Vector2i(16, 16));
            }
            if (ZPClientGunClientTickProcessing.showRightMouseIndicator(mc)) {
                ZPGunsUIEvent.tip(graphics, ZPGunsUIEvent.mouse_right, new Vector2i(center.x, 24).sub(-8, 16), new Vector2i(16, 16), new Vector2i(16, 16));
            }
            if (ZPClientGunClientTickProcessing.showPistolIndicator(mc)) {
                ZPGunsUIEvent.tip(graphics, ZPGunsUIEvent.pistol_ind, new Vector2i(center.x, 24).sub(8, 16), new Vector2i(16, 16), new Vector2i(16, 16));
            }
            ...
            if (itemStackInRightHand.getItem() instanceof ZPBaseGun zpBaseGun) {
                ZPGunsUIEvent.renderDataGun(mc, graphics, itemStackInRightHand, player, zpBaseGun, width - mc.font.width(zpBaseGun.getName(itemStackInLeftHand)) - 60, 60, true);
            }

            if (itemStackInLeftHand.getItem() instanceof ZPBaseGun zpBaseGun) {
                ZPGunsUIEvent.renderDataGun(mc, graphics, itemStackInLeftHand, player, zpBaseGun, 60, 60, false);
            }
        }
    }
