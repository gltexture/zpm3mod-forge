...
public final class ZPLootNbtRandomFloat extends ZPLootNbtValue {
    private final float min;
    private final float max;
    private final ZPRandomization randomization;

    public ZPLootNbtRandomFloat(float min, float max, @NotNull ZPRandomization randomization) {
        super(ZPLootNbtValue.TYPE_RANDOM_FLOAT);
        this.min = min;
        this.max = max;
    ...
    public void writeValue(@NotNull CompoundTag nbt, @NotNull String key) {
        nbt.putFloat(key, this.randomization.random(this.min, this.max));
    }
