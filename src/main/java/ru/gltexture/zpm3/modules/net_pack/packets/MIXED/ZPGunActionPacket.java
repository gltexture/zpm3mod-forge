...
public class ZPGunActionPacket implements ZPNetwork.ZPPacket {
    ...
    private void sendPacketFromServer(@NotNull Player sender, @NotNull ServerLevel serverLevel) {
        ZombiePlague3.netServer().sendToDimensionRadius(new ZPGunActionPacket(sender.getId(), this.action, this.isRightHand), serverLevel.dimension(), sender.position(), ZPNetworkConfig.GUN_ACTION_PACKET_RANGE.getVar());
    }
