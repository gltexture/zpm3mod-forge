...
public class ZPSendTheOnlyZone_Packet implements ZPNetwork.ZPPacket {
    public static final String SEPARATOR = ";";
    public static final String REM_FLAG = "$NULL$";
    private final ZPZoneManager.Zone zone;

    public ZPSendTheOnlyZone_Packet(ZPZoneManager.Zone zone) {
        this.zone = zone;
    }

    public ZPSendTheOnlyZone_Packet(FriendlyByteBuf buf) {
    ...
    public static Encoder<ZPSendTheOnlyZone_Packet> encoder() {
        return (packet, buf) -> {
    ...
    public static Decoder<ZPSendTheOnlyZone_Packet> decoder() {
        return ZPSendTheOnlyZone_Packet::new;
    }
    ...
    public void onClient(@NotNull Player player) {
        if (this.zone.uniqueId().startsWith(ZPSendTheOnlyZone_Packet.REM_FLAG)) {
            ZPZoneManager.INSTANCE.REMOVE_ZONE_FROM_CLIENT_MAP((ClientLevel) player.level(), this.zone.uniqueId().substring(ZPSendTheOnlyZone_Packet.REM_FLAG.length()));
        } else {
